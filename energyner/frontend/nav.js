 ...
 * de envío de datos, manipularlo y  conectarnos al puerto 3001 del servidor local que hemos configurado */
...
async function calcularConsumoEnergetico(potencia, horas) {
    const response = await fetch('http://localhost:3001/api/consumo-energetico', {
        method: 'POST',
        headers: {
...
async function calcularProduccionSolar(area, irradiacion, eficiencia) {
    const response = await fetch('http://localhost:3001/api/produccion-solar', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
...
document.getElementById("calcular").addEventListener("click", async function (event) {
    ...
    try {
        ...
        const response = await fetch("http://localhost:3001/api/huella-carbono", {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
            },
